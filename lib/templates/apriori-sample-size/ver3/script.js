...
class SampleSizeEstimator {
  ...
  bindAdvancedToggle(this.stratAdvancedToggle, this.stratAdvanced);
    ...
    // Code action buttons (copy, download, print)
    ...
    const printCodeBtn = document.getElementById('printCode');
    if (printCodeBtn) {
      printCodeBtn.addEventListener('click', () => this.printCode());
    }
    ...
    if (downloadOutputBtn) {
      downloadOutputBtn.addEventListener('click', () => this.downloadOutput());
    }

    const printOutputBtn = document.getElementById('printOutput');
    if (printOutputBtn) {
      printOutputBtn.addEventListener('click', () => this.printOutput());
    }
  ...
  copyCode() {
    ...
    navigator.clipboard.writeText(code).then(() => {
      ...
      copyButton.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"></path></svg>';
  ...
  downloadOutput() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'sample_size_output.txt';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  printCode() {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>R Code - Sample Size Calculator</title>
          <style>
            body { 
  ...
  printOutput() {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>R Output - Sample Size Calculator</title>
          <style>
            body { 
              font-family: 'Courier New', monospace; 
              padding: 20px; 
...
document.addEventListener('DOMContentLoaded', () => {
  // Detect if this page is the base Sample Size Estimator (has effect size/power controls)
  const isBaseEstimatorPage = !!(document.getElementById('effectSize') || document.getElementById('statisticalPower') || document.getElementById('testType'));

  // Always initialize info modal system
  initInfoModal();

  if (isBaseEstimatorPage) {
    const app = new SampleSizeEstimator();
    // Expose app instance globally for console access
    window.sampleSizeApp = app;

    // Run validation tests specific to base estimator
...
// ==============================
// (Removed) Header gradient variant selector — restoring original static header
