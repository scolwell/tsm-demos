...
class SampleSizeEstimator {
  ...
  const bindRecalc = (el) => el && el.addEventListener('input', () => { this.markInteracted(); this.updateCalculations(); });
    ...
    if (this.vizResample) this.vizResample.addEventListener('click', () => {
      // Manual resampling: redraw visualization with new random sample
      this.markInteracted();
      this.updateSamplingVisualization();
    });
  ...
  updateSamplingVisualization() {
    ...
    const iccBadge = document.getElementById('iccBadge');
    ...
    // Sample badge - show for all non-systematic cases
    if (sampBadge) {
      if (method !== 'systematic' || design !== 'no-stratification') {
        sampBadge.textContent = `Sample: n = ${desiredSample}`;
        sampBadge.style.display = 'inline-flex';
      } else {
        sampBadge.style.display = 'none';
      }
    }
    
    // Systematic badge - show only for systematic sampling with no stratification
    if (sysBadge) {
      if (method === 'systematic' && design === 'no-stratification') {
    ...
    // Cluster badge - show for cluster or stratified-cluster designs
    if (clusterBadge) {
      if (design === 'cluster' || design === 'stratified-cluster') {
        const numClusters = design === 'cluster' ? 
          parseInt(this.numClusters?.value) || 40 :
          parseInt(this.sc_numClusters?.value) || 40;
        const avgClusterSize = Math.ceil(desiredSample / numClusters);
        clusterBadge.textContent = `c̄=${avgClusterSize}`;
    ...
    // ICC badge - show for cluster or stratified-cluster designs
    if (iccBadge) {
      if (design === 'cluster' || design === 'stratified-cluster') {
        const icc = design === 'cluster' ?
          parseFloat(this.clusterICC?.value) || 0.05 :
          parseFloat(this.sc_clusterICC?.value) || 0.05;
        iccBadge.textContent = `ICC=${icc.toFixed(3)}`;
        iccBadge.style.display = 'inline-flex';
      } else {
        iccBadge.style.display = 'none';
      }
    }
    
    // Allocation badge - show for stratified or stratified-cluster designs
    if (allocationBadge) {
      if (design === 'stratified' || design === 'stratified-cluster') {
        const allocMode = design === 'stratified' ?
          this.currentValues.strataAllocMode || 'proportional' :
          this.currentValues.sc_strataAllocMode || 'proportional';
        const allocText = allocMode === 'proportional' ? 'Proportional' : 'Equal';
        allocationBadge.textContent = allocText;
        allocationBadge.style.display = 'inline-flex';
      } else {
        allocationBadge.style.display = 'none';
      }
    }
